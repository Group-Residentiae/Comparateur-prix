(() => {
  try {
    ...
    // --- CSV URL (obligatoire)
    const CSV_URL =
      mount.getAttribute("data-csv-url") ||
      ""; // laisse vide => on affichera un message

    // --- Trouver l'URL DU script widget.js (fiable même en embed)
    function getWidgetScriptUrl() {
      // 1) currentScript si dispo
      if (document.currentScript && document.currentScript.src) return document.currentScript.src;

      // 2) sinon, chercher un <script> qui contient "widget.js"
      const scripts = Array.from(document.querySelectorAll("script[src]"));
      const hit =
        scripts.find((s) => /\/widget\.js(\?|#|$)/i.test(s.getAttribute("src") || "")) ||
        scripts.find((s) => (s.getAttribute("src") || "").toLowerCase().includes("widget.js"));

      return hit ? hit.getAttribute("src") : "";
    }

    const WIDGET_SCRIPT_SRC = getWidgetScriptUrl();
    const WIDGET_SCRIPT_URL = (() => {
      try {
        return new URL(WIDGET_SCRIPT_SRC || location.href, location.href);
      } catch {
        return new URL(location.href);
      }
    })();

    // Base icons: par défaut relative à widget.js => .../assets/icons/
    const ICONS_BASE =
      mount.getAttribute("data-icons-base") ||
      new URL("./assets/icons/", WIDGET_SCRIPT_URL).href;
    ...
    function iconForProduct(productName) {
      ...
      if (k.includes("recuperateur") && k.includes("pluie")) return new URL("recuperateur-pluie.png", ICONS_BASE).href;
      if (k.includes("rouleau") && k.includes("gazon")) return new URL("rouleau-gazon.png", ICONS_BASE).href;
      return "";
    }

    // --- Render shell (TOUJOURS)
    mount.innerHTML = `
      ...
      carousel.innerHTML = list.map((r) => {
        ...
        const mediaHtml = iconSrc ? `
          <div class="pcw-media" aria-hidden="true">
            <img src="${escapeAttr(iconSrc)}" alt="" loading="lazy" decoding="async">
          </div>
    ...
    // --- STOP si pas de CSV_URL
    if (!CSV_URL) {
      $('[data-slot="carousel"]').innerHTML =
        `<div class="pcw-state"><strong>CSV_URL manquant.</strong><br/>Ajoute <code>data-csv-url</code> sur <code>.pcw-garden-widget</code>.</div>`;
      setCount("—");
      postHeight();
      return;
    }
    ...
    (async () => {
      try {
        ...
        $('[data-slot="carousel"]').innerHTML =
          `<div class="pcw-state"><strong>Impossible de charger le CSV.</strong><br/>Vérifie l’URL, CORS, et la console.</div>`;
        setCount("—");
        postHeight();
      }
    })();
  } catch (e) {
    console.error("[PCW] fatal:", e);
  }
})();
